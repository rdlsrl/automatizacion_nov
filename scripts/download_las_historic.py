...
def take_screenshot(name):
    screenshot_dir = "/mnt/mariadb/autom_nov/scripts/screenshots"
    if not os.path.exists(screenshot_dir):
        os.makedirs(screenshot_dir)
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    path = os.path.join(screenshot_dir, f"{name}_{timestamp}.png")
    driver.save_screenshot(path)
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Captura guardada: {path}")


def save_page_source(name):
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    path = f"/mnt/mariadb/autom_nov/scripts/{name}_{timestamp}.html"
    with open(path, "w", encoding="utf-8") as f:
        f.write(driver.page_source)
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Código fuente guardado: {path}")
...
def login_to_welldata():
    ...
    try:
        ...
        take_screenshot("error_login")  # Captura de pantalla en caso de error
        save_page_source("error_login")  # Guardar el código fuente de la página
        raise
...
def select_contractor_and_rig(contractor, rig):
    try:
        ...
        contractor_option = wait.until(EC.presence_of_element_located(
            (By.XPATH,
             f"//option[contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '{contractor}')]")
        ))
        ...
        take_screenshot("error_seleccion_equipo")
        save_page_source("error_seleccion_equipo")
        raise
...
def procesar_descarga(expected_well, rig_name, fecha_inicial_download, fecha_final_download):
    ...
    try:
        ...
        take_screenshot("error_config_canales")  # Cambiado nombre de captura
        save_page_source("error_config_canales")  # Cambiado nombre de captura
        driver.quit()
        sys.exit(1)
    ...
    try:
        ...
        take_screenshot("error_fechas_pozo")
        save_page_source("error_fechas_pozo")
        driver.quit()
        sys.exit(1)
    ...
    try:
        ...
        take_screenshot("error_fechas_rigs_download")
        save_page_source("error_fechas_rigs_download")
        driver.quit()
        sys.exit(1)
    ...
    if len(windows_before) > 1:
        ...
        try:
            ...
            take_screenshot("error_yes_button")
            save_page_source("error_yes_button")
        time.sleep(10)
        ...
        try:
            ...
            take_screenshot("error_save_file_now")
            save_page_source("error_save_file_now")
        time.sleep(5)
    else:
    ...
    if las_files:
        ...
        if suffix == "_nws":
            new_filename = f"{rig_name}_{expected_well}_{fecha_hora}_nws.las"
        else:
        ...
        take_screenshot("error_no_las_file")
        save_page_source("error_no_las_file")
...
def procesar_equipo(contractor, rig, expected_well, expected_spud_date, expected_rr, rig_name,
                     fecha_inicial_download, fecha_final_download):
    try:
...
def main():
    try:
        ...
        for rec in registros_wd:
            ...
            procesar_equipo(contractor, rig, expected_well, expected_spud_date, expected_rr, rig_name, fecha_inicial,
                            fecha_final)
            print("-----------------------------------------------------\n")
    except Exception as e:
        print("Error en el proceso global.")
        take_screenshot("error_global")
        save_page_source("error_global")
    finally:
        driver.quit()
